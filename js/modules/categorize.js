 ...
 class movieFilter {
  ...
  static async filterFavMovie(){
    try {
      ...
      const favMovieSlug = [
        'nguu-lang-chuc-nu', 'nguoi-hung-yeu-duoi-2', 'khi-cuoc-doi-cho-ban-qua-quyt',
        'rung-khong-tieng', 'the-gioi-ma-quai-phan-3', 'kho-do-danh'
      ];
  ...
  static async filterContinuteMovie(){
    try {
      ...
      const continuteMovieSlug = [
        'tinh-yeu-cua-mu-bi', 'dua-hau-lap-lanh', 'doona',
        'thanh-xuan-182-lu-trinh-huong-ve-em', 'suyt-quoc-vuong-dang-ngu-dong',
      ];
      
      const continutePromises = continuteMovieSlug.map(slug => fetchMovieDetail(slug));
      const continuteListDetails = await Promise.all(continutePromises);

      const validContinuteDetails = continuteListDetails.filter(detail => detail !== null);
      
      if (continuteListDetails && continuteListDetails.length > 0) {
        const continuteList = validContinuteDetails.map(detail => new MovieDetail(detail));
...
catagorMovie.korea = {
 ...
 // Phân loại khác
    ...
    catagorMovie.randomMovies = movieFilter.filterRandomMovies(fullMovieList, 10);
